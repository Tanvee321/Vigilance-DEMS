...
/**
 * Computes SHA-256 hash string natively using browser Progressive FileReader chunking (10MB chunks)
 */
...
export const CATEGORY_MAP: Record<string, string[]> = {
  'Disk Image (Forensic Level)': ['.raw', '.aff4', '.dd', '.e01'],
  'Network Traffic Captures': ['.pcap', '.pcapng', '.log'],
  'Volatile Memory Dumps': ['.raw', '.dmp', '.vmem'],
  'Encrypted Storage Volume': ['.iso', '.vmdk', '.ova'],
  'Logical File Collection': ['.txt', '.log']
};

export function getCategoryPlaceholder(category: string): string {
  const norm = category ? category.toLowerCase() : '';
  if (norm.includes('disk image')) {
    return 'Drag & Drop Forensic Disk Asset (.raw, .aff4, .dd, .e01)';
  }
  if (norm.includes('network traffic')) {
    return 'Drag & Drop Packet Capture Payload (.pcap, .pcapng, .log)';
  }
  if (norm.includes('volatile memory')) {
    return 'Drag & Drop Volatile RAM Dump File (.raw, .dmp, .vmem)';
  }
  if (norm.includes('encrypted storage')) {
    return 'Drag & Drop Virtual Disk Container (.iso, .vmdk, .ova)';
  }
  if (norm.includes('logical')) {
    return 'Drag & Drop Flat Log Extract Document (.txt, .log)';
  }
  return 'Drag & Drop Forensic File Here';
}
