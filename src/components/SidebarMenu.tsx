...
export const SidebarMenu: React.FC<SidebarMenuProps> = ({
  ...
  return (
    <>
          ...
          <>
            ...
            >
              ...
              <div className="mt-auto px-3 space-y-1">
                ...
                <div 
                  onClick={() => {
                    alert("Logging out from investigator console node.");
                    setIsMenuOpen(false);
                  }}
                  className="flex items-center gap-4 px-4 py-3 rounded text-[#c5c6cd] hover:text-white hover:bg-[#343536] transition-all duration-200 cursor-pointer"
                >
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-[#0b0e14] border-r border-outline-variant/60 h-full p-6 shrink-0 justify-between">
        <div className="space-y-4">
          <div className="text-[10px] font-mono text-[#c5c6cd]/40 uppercase tracking-widest px-4 mb-4 select-none font-bold">
            Console Navigation
          </div>
          <nav className="space-y-1">
            <button 
              onClick={() => handleTabClick('dashboard')}
              className={`w-full flex items-center gap-4 px-4 py-3 rounded-lg text-left transition-all duration-200 cursor-pointer border ${
                activeView === 'dashboard' 
                  ? 'text-tertiary bg-tertiary/10 border-tertiary/30 font-bold shadow-[0_0_15px_rgba(56,222,187,0.05)]' 
                  : 'text-[#c5c6cd] hover:text-white hover:bg-[#343536] border-transparent'
              }`}
            >
              <span className="material-symbols-outlined text-[20px]">dashboard</span>
              <span className="font-label-caps text-xs tracking-wider uppercase font-bold">Dashboard</span>
            </button>
            ...
            >
              ...
              <span className="font-label-caps text-xs tracking-wider uppercase font-bold">Archive</span>
            </button>
            ...
            >
              ...
              <span className="font-label-caps text-xs tracking-wider uppercase font-bold">Forensics</span>
            </button>
            ...
            >
              ...
              <span className="font-label-caps text-xs tracking-wider uppercase font-bold">Audit Logs</span>
            </button>
            ...
            >
              ...
              <span className="font-label-caps text-xs tracking-wider uppercase font-bold">Settings</span>
            </button>
          </nav>
        </div>

        <div className="pt-6 border-t border-[#44474d]/30 space-y-4">
          <button 
            onClick={() => executeLockoutTerminal()}
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-[#93000a]/20 text-error font-mono text-xs border border-error/20 hover:bg-[#93000a]/40 transition-colors cursor-pointer"
          >
            <span className="material-symbols-outlined text-[18px]">lock_person</span>
            Emergency Lockout
          </button>
          <div 
            onClick={() => openTerminalNotice(
              'CLEARANCE SHA1 -> SHA256 CONVERSION MODULE',
              'Sovereign support module to anchor older SHA-1 hash structures into SHA-256 standard cryptographic trees for Sentinel Mainnet archival compatibility. Requires Level 5 Security clearance token payload to authorize database transaction rewrite.',
              [
                `[DIAGNOSTICS] SHA-1 legacy database mapped: 4,212 archives detected.`,
            ...
            className="flex items-center gap-4 px-4 py-2.5 rounded-lg hover:text-white hover:bg-[#343536] transition-all duration-200 cursor-pointer text-[#c5c6cd] text-xs font-mono font-bold"
          >
          ...
          <div 
            onClick={() => openTerminalNotice(
              'SECURE INVESTIGATOR LOGGER', 
              'Deauthorizing hardware cryptographic key bundle and closing the secure session connection cleanly. All buffered directories are securely wiped from in-memory state.', 
              [
                '[LOGGER] Revoking L5 keys node registration...', 
                '[SHUTDOWN] Clearing fast registers memory...', 
                '[SUCCESS] Safe to close browser tab.'
              ], 
              'LEVEL 5 CONSOLE LOGOUT'
            )}
            className="flex items-center gap-4 px-4 py-2.5 rounded-lg hover:text-white hover:bg-[#343536] transition-all duration-200 cursor-pointer text-[#c5c6cd] text-xs font-mono font-bold"
          >
