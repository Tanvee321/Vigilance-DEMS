...
export const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-10 gap-6 items-stretch pb-20">
        
        {/* Left column Settings panel - col span 6 */}
        <div className="lg:col-span-6 flex flex-col justify-between glass-panel rounded-xl overflow-hidden border border-outline-variant shadow-2xl">
        ...
        <div className="lg:col-span-4 flex flex-col justify-between glass-panel rounded-xl overflow-hidden border border-outline-variant shadow-2xl p-6 space-y-6">
          ...
          <div className="bg-[#d97706]/10 border border-[#f59e0b]/20 p-4 rounded-xl space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                type="button"
                onClick={() => {
                  if (setIsConnectionInterrupted) {
                    setIsConnectionInterrupted(!isConnectionInterrupted);
                  }
                }}
                className={`py-2 px-1 rounded text-[9px] font-label-caps uppercase font-bold transition-all border outline-none cursor-pointer ${
                  isConnectionInterrupted
                    ? 'bg-amber-500/20 text-amber-500 border-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.2)]'
                    : 'bg-black/30 text-[#fafafa]/80 border-white/5 hover:border-amber-500/40 hover:bg-black/50'
                }`}
              >
                {isConnectionInterrupted ? 'DISMISS AMBER' : 'TEST TIMEOUT BANNER'}
              </button>
            </div>
          </div>
        </div>
